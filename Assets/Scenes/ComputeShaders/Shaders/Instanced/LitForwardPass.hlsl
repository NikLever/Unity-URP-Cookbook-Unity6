...
Varyings LitPassVertex(Attributes input)
{
    ...
    float4x4 mat = create_matrix(boidsBuffer[unity_InstanceID].position, boidsBuffer[unity_InstanceID].direction, float3(0.0, 1.0, 0.0));

    object2world = mul(mat, object2world);
    // 行列に位置（平行移動）を適用
